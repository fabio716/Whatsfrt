...
export default function AdminNav({ userRole }: Readonly<{ userRole: Role }>) {
  ...
  const visibleItems = NAV_ITEMS.filter((item) =>
    (item.roles as readonly Role[]).includes(userRole)
  )

  return (
    <nav className="flex w-52 flex-shrink-0 flex-col border-r border-zinc-100 bg-white">
      {/* Brand */}
        ...
        {visibleItems.map((item) => {
          ...
          return (
            <a
              key={item.href}
              href={item.href}
              className={`flex items-center gap-2.5 rounded-lg px-3 py-2 text-[12.5px] font-medium transition-colors ${
                isActive
      ...
      <div className="border-t border-zinc-100 px-2 py-3">
        <button
          ...
          className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-[12.5px] font-medium text-zinc-400 transition-colors hover:bg-red-50 hover:text-red-500"
        >
          <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth={2}>
